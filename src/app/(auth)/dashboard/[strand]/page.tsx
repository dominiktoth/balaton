...
export default function DashboardPage() {
  ...
  const { data: stores, isLoading } = api.store.getAllStores.useQuery({ strandSlug });
  const { data: expenses, refetch: refetchExpenses } =
    api.expense.getAllExpenses.useQuery({ strandSlug });
  const { data: allOrders = [] } = api.order.getStoreOrders.useQuery({
    storeId: selectedStoreId,
  });

  // Fetch incomes
  const { data: allIncomes = [], refetch: refetchAllIncomes } = api.income.getAllIncomes.useQuery({ strandSlug }, {
  ...
  // Fetch workers
  const { data: workers = [], refetch: refetchWorkers } = api.worker.getWorkers.useQuery({ strandSlug });
